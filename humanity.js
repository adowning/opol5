	...
	return new Promise(resolve =>
		axios
			.get(
				...
				`${humanityBase}/employees/${requestData.userId}?access_token=${
					requestData.token
				}`
			)
			.catch(error => {
...
const getTimeClocks = (requestData, callback) => {
	return new Promise(resolve =>
		axios
			.get(`${humanityBase}/timeclocks?access_token=${requestData.token}`, {
				params: requestData
			})
			.catch(error => {
				resolve(error)
		...
		axios
			.post(
				`https://www.humanity.com/api/v2/employees/${
					requestData.userId
				}/clockin?access_token=${requestData.token}`
			)
			.then(res => {
				console.log(res.data)
				const response = {
	...
	return new Promise(resolve =>
		axios
			.put(
				`https://www.humanity.com/api/v2/employees/${
					requestData.userId
				}/clockout?access_token=${requestData.token}`
			)
...
const createSnipeitUser = (requestData, callback) => {
	return new Promise(resolve =>
		snipeit
			.post(`http://192.168.1.171:83/api/v1/users`, {
